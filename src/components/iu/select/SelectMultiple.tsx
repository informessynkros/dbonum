...
import { Check, ChevronDown, Search, CircleX, AlertCircle } from "lucide-react"
...
function SelectMultiple<T extends Option>({
  ...
  // Animación para alertar sobre el máximo de seleccionado
  useEffect(() => {
    if (maxSelectionWarning && warningRef.current) {
      ...
      const timer = setTimeout(() => {
        if (warningRef.current) {
          gsap.to(warningRef.current, {
            ...
            duration: 0.2,
            ease: "power2.in",
            onComplete: () => setMaxSelectionWarning(false)
          })
        }
      }, 2000)
  ...
  // Crear placeholder dinámico basado en maxSelection
  const getPlaceholder = () => {
    if (selectedOptions.length > 0) {
      if (multiple) {
        const count = selectedOptions.length
        const maxText = maxSelection ? `/${maxSelection}` : ''
        return `${count}${maxText} seleccionado${count !== 1 ? "s" : ""}`
      } else {
        return String(selectedOptions[0][labelKey])
      }
    }

    if (multiple && maxSelection) {
      return `${placeholder} (máx ${maxSelection})`
    }
  ...
  return (
    <div className="relative w-full mb-4" ref={selectRef}>
      ...
      {maxSelectionWarning && (
        <div
          ref={warningRef}
          className="flex justify-center items-center text-orange-700 text-sm mt-1 bg-orange-100 font-semibold rounded-md p-2 mb-2 gap-2"
          style={{ opacity: 0, transform: 'scale(0.8)' }}
        >
          <AlertCircle className="w-4 h-4" />
          Máximo {maxSelection} selecciones permitidas
        </div>
      )}
      ...
      <button
        ...
        className="w-full flex items-center justify-between px-4 py-2 text-sm bg-white border rounded-lg focus:outline-none focus:ring-2 cursor-pointer border-gray-300 focus:ring-slate-700 transition-colors duration-200"
        disabled={disabled}
        type="button"
      ...
      {isOpen && (
        <>
          ...
          <div
            ref={backdropRef}
            className="fixed inset-0 z_index_3"
          ...
          <div
            ref={dropdownRef}
            className={`absolute z_index_4 bg-white rounded-lg shadow-lg border border-gray-200 w-full ${dropdownDirection === "up" ? "bottom-full mb-2" : "top-full mt-2"
              }`}
            style={{
              minWidth: "250px",
              maxHeight: "320px",
                ...
                filteredOptions.map((option, index) => {
                  ...
                  return (
                    <div
                      ...
                      className={`flex items-center px-4 py-2 transition-colors duration-150 ${isDisabled
                    ...
                    >
                      <div
                        className={`w-4 h-4 border rounded mr-3 flex items-center justify-center transition-all duration-200 ${isOptionSelected
                      ...
                      >
                        {isOptionSelected && (
                          <Check
                            ...
                            style={{
                              transform: 'scale(0)',
                              animation: isOptionSelected ? 'checkAppear 0.2s ease-out forwards' : 'none'
                            }}
                          />
                        )}
                      </div>
